...
Deno.serve(async (req) => {
  try {
    ...
    const { runId, enabled } = await req.json();
    ...
    const forceNextEventRelic = !!enabled;
    const devFlags = { ...(progress.devFlags ?? {}), forceNextEventRelic };
    const nextIdx = (run.nextActionIdx ?? 0) + 1;

    await Promise.all([
      base44.entities.Run.update(runId, {
        results: { ...(run.results ?? {}), progress: { ...progress, devFlags } },
        nextActionIdx: nextIdx,
      }),
      base44.entities.RunAction.create({ runId, idx: nextIdx, actionType: 'dev_force_event_relic', payload: { enabled: forceNextEventRelic } }),
    ]);

    return Response.json({ ok: true, forceNextEventRelic });
  } catch (error) {
